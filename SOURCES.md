...
1. Add a keyed entry here (`[NewKey]`) with author, year, title, venue, and a locator (DOI/URL/page).
2. Cite it inline as `[NewKey]` next to the claim it supports.
3. If you can't produce a real locator, the claim it would support cannot be tagged `[ESTABLISHED]`/`[SUPPORTED]`/`[CONTESTED]` — downgrade it to `[OPEN]` or `[SPECULATIVE]` instead. See [`TAXONOMY.md`](./TAXONOMY.md).
